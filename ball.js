function Ball() {
  ...
  this.bounce = function () {
   ...
   this.distroyLevelOne = function() {
    ...
    if(this.x >= breaks[i].x - breaks[i].length/2 - this.rad/2 && this.x <= breaks[i].x + breaks[i].length/2 + this.rad/2){
      if(this.y > breaks[i].y){
        if(this.y <= breaks[i].y + this.rad/2 + breaks[i].height/2 + 5 && this.y >= breaks[i].y + this.rad/2 + breaks[i].height/2 - 10){
          this.yspeed *= -1;
          this.distroyOne();
        }
      }
      else if(this.y < breaks[i].y){
        if(this.y >= breaks[i].y - this.rad/2 - breaks[i].height/2 - 5 && this.y <= breaks[i].y - this.rad/2 - breaks[i].height/2 + 10){
          this.yspeed *= -1;
          this.distroyOne();
        }
      ...
      if(this.x > breaks[i].x){
        if(this.x <= breaks[i].x + this.rad/2 + breaks[i].length/2 + 5 && this.x >= breaks[i].x + this.rad/2 + breaks[i].length/2 - 10){
          this.xspeed *= -1;
          this.distroyOne();
        }
      }
      else if(this.x < breaks[i].x){
        if(this.x >= breaks[i].x - this.rad/2 - breaks[i].length/2 && this.x + 5<= breaks[i].x - this.rad/2 -breaks[i].length/2 + 10){
          this.xspeed *= -1;
          this.distroyOne();
        }
      }
  ...
  this.distroyLevelTwo = function() {
    ...
    if(this.x >= bar[j].x - bar[j].length/2 - this.rad/2 && this.x <= bar[j].x + bar[j].length/2 + this.rad/2){
      if(this.y > bar[j].y){
        if(this.y <= bar[j].y + this.rad/2 + bar[j].height/2 + 5 && this.y >= bar[j].y + this.rad/2 + bar[j].height/2 - 10){
          this.yspeed *= -1;
          this.distroyTwo();
        }
      }
      else if(this.y < bar[j].y){
        if(this.y >= bar[j].y - this.rad/2 - bar[j].height/2 - 5 && this.y <= bar[j].y - this.rad/2 - bar[j].height/2 + 10){
          this.yspeed *= -1;
          this.distroyTwo();
        }
    ...
    else if(this.y >= bar[j].y - bar[j].height/2 - this.rad/2 && this.y <= bar[j].y + bar[j].height/2 + this.rad/2){
      if(this.x > bar[j].x){
        if(this.x <= bar[j].x + this.rad/2 + bar[j].length/2 + 5 && this.x >= bar[j].x + this.rad/2 + bar[j].length/2 - 10){
          this.xspeed *= -1;
          this.distroyTwo();
        }
      }
      else if(this.x < bar[j].x){
        if(this.x >= bar[j].x - this.rad/2 - bar[j].length/2 && this.x + 5<= bar[j].x - this.rad/2 -bar[j].length/2 + 10){
          this.xspeed *= -1;
          this.distroyTwo();
        }
      }
  ...
  this.distroyLevelThree = function() {
    ...
    if(this.x >= blocks[h].x - blocks[h].length/2 - this.rad/2 && this.x <= blocks[h].x + blocks[h].length/2 + this.rad/2){
      if(this.y > blocks[h].y){
        if(this.y <= blocks[h].y + this.rad/2 + blocks[h].height/2 + 5 && this.y >= blocks[h].y + this.rad/2 + blocks[h].height/2 - 10){
          this.yspeed *= -1;
          this.distroyThree();
        }
      }
      else if(this.y < blocks[h].y){
        if(this.y >= blocks[h].y - this.rad/2 - blocks[h].height/2 - 5 && this.y <= blocks[h].y - this.rad/2 - blocks[h].height/2 + 10){
          this.yspeed *= -1;
          this.distroyThree();
        }
    ...
    else if(this.y >= blocks[h].y - blocks[h].height/2 - this.rad/2 && this.y <= blocks[h].y + blocks[h].height/2 + this.rad/2){
      if(this.x > blocks[h].x){
        if(this.x <= blocks[h].x + this.rad/2 + blocks[h].length/2 + 5 && this.x >= blocks[h].x + this.rad/2 + blocks[h].length/2 - 10){
          this.xspeed *= -1;
          this.distroyThree();
        }
      }
      else if(this.x < blocks[h].x){
        if(this.x >= blocks[h].x - this.rad/2 - blocks[h].length/2 && this.x + 5<= blocks[h].x - this.rad/2 - blocks[h].length/2 + 10){
          this.xspeed *= -1;
          this.distroyThree();
        }
      }
